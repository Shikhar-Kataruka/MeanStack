...
function regionCreation(object) {
    ...
    var newRegion = new region({  //new region object
        ...
        religion: religion.slice(Math.floor((Math.random() * (0 - 3) + 0)), Math.floor((Math.random() * (3 - 7) + 3))),
        language: language.slice(Math.floor((Math.random() * (0 - 5) + 0)), Math.floor((Math.random() * (5 - 9) + 5)))
    });
    newRegion.save().then(savedregion => {
        for(var stateObject of object.stateArray) {
             stateCreation(stateObject, savedregion);
        }
        })  
     // saved region object
    
}
function stateCreation(stateObject,savedregion) {
    var stateName = stateObject;
        ...
        newState.save().then(savedstate=>{
            ...
            for(const cityObject of cityArray[0].city){
                cityCreation(cityObject, savedstate, savedregion);
            }
        }); // saved region object
...
function cityCreation(cityObject,savedstate,savedregion) {
    var cityname = cityObject;
            ...
            const newCity = new city({
                ...
                religion: religion.slice(Math.floor((Math.random() * (0 - 3) + 0)), Math.floor((Math.random() * (3 - 7) + 3))),
                language: language.slice(Math.floor((Math.random() * (0 - 5) + 0)), Math.floor((Math.random() * (5 - 9) + 5))),
                pincode: stringHash(savedregion.name + cityname + savedstate.name),
                stateId: savedstate._id
            });
            newCity.save().then(savedcity => {
                var schoolrandom = Math.floor((Math.random() * 5) + 1);  //creating 5 or less schools
                for (let i = 0; i < schoolrandom; i++) {
                  return  schoolCreation(savedregion, savedstate, savedcity);
            }
     }); // saved city object
            
}

function schoolCreation(savedregion, savedstate, savedcity) {
    var schoolname = savedcity.name + " public " + "school";
    var newSchool = new school({
        name: schoolname,
        contact: {
            email: savedcity.name + "publicschool@gmail.com",
            phoneNo: counter
        },
        address: {
            region: { name: savedregion.name, id: savedregion._id },
            state: { name: savedstate.name, id: savedstate._id },
            city: { name: savedcity.name, id: savedcity._id }
        }
    });
    newSchool.save().then(savedschool => {
        return  studentCreation(savedregion, savedstate, savedcity);
    }); // saved city object 
    
}
async function studentCreation(savedregion, savedstate, savedcity) {
    counter++;
